...
def print_table():
    columns =['language', 'user_id', 'compiled', 'exit_code']
...
def print_table_self_contained():
    columns =['language', 'user_id', 'compiled', 'exit_code']
    language = ['C', 'C#', 'C++', 'Java','Python']
    data = get_all_data(columns)
    columns =['languageuser_id', 'compiled', 'exit_code']
    ...
    all_strings = []
    overall_values = [0, 0, 0]
    for i,l in enumerate(language) :
        string = l 
        string = string +  ' & ' + str(frames[1].iloc[i]['compiled']) +  ' & ' + str(int((float(frames[1].iloc[i]['compiled'])) / (float(frames[0].iloc[i]['user_id']))*100)) +  ' & ' + str(frames[2].iloc[i]['exit_code']) +  ' & ' + str(int((float(frames[2].iloc[i]['exit_code']))/(float(frames[0].iloc[i]['user_id']))*100))
        overall_values[0] = overall_values[0] + frames[1].iloc[i]['compiled']
        overall_values[1] = overall_values[1] + frames[0].iloc[i]['user_id']
        overall_values[2] = overall_values[2] + frames[2].iloc[i]['exit_code']
        string = string + '\\\\'
    ...
    print '\\toprule'
    #print     'Language & Number Compile Errors & \% Compile Error  & Number Runtime Errors & \% Runtime Errors \\\\'
    print     'Language & Compile Errors & \% Compile Error  & Runtime Errors & \% Runtime Errors \\\\'
    print     '\\midrule'
    print all_strings[0]
    print all_strings[1]
    print all_strings[2]
    ...
    print all_strings[4]
    print '    \\midrule'
    print '    Total & ' + str(overall_values[0]) + ' & ' + str(int(float(overall_values[0]/float(overall_values[1]))*100)) + ' & ' + str(overall_values[2]) + ' & ' + str(int(float(overall_values[2]/float(overall_values[1])*100))) + '\\\\'
    print '\\bottomrule'
    print '\\end{tabular}'
    print '\\end{table}'
